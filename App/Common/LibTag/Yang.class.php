...
class Yang extends TagLib
{
    ...
    protected $tags = array(
        ...
        //栏目
        'catlist'         => array(
            'attr'  => 'typeid,type,orderby,limit,flag,modelid,typecid', //flag为是否全部显示
            'close' => 1,
        ),
...
str;
    ...
    {
        $typeid  = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //只接收一个栏目ID
        $typepid  = !isset($attr['typepid']) || $attr['typepid'] == '' ? -1 : trim($attr['typepid']); //只接收一个栏目ID
        ...
        $limit    = empty($attr['limit']) ? '10' : $attr['limit'];
        $orderby = empty($attr['orderby']) ? 'id DESC' : $attr['orderby'];
        $typecid = !isset($attr['typecid']) || $attr['typecid'] == '' ? -1 : $attr['typecid'];
        $modelid = empty($attr['modelid']) ? 0 : intval($attr['modelid']);
        $str     = <<<str
<?php
	\$_typeid = intval($typeid);
	\$_typepid = intval($typepid);
	\$_type = "$type";
	\$_temp = explode(',', "$limit");
...
str;
    ...
    public function _navlist($attr, $content)
    {
        ...
        $typepid = !isset($attr['typepid']) || $attr['typepid'] == '' ? -1 : $attr['typepid'];
        $str    = <<<str
<?php
	\$_typeid = $typeid;
	\$_cid = $typepid;
	...
	foreach(\$_navlist as \$autoindex => \$navlist):
		...
		if(\$navlist['id'] == \$_cid || in_array(\$navlist['id'], \$pid_arr)){
		  	\$navlist['class'] = 'on';
		}
?>
str;
...
str;
    ...
    public function _publicty($attr, $content)
    {
    	return C('CFG_PUBLICTY');
    }
